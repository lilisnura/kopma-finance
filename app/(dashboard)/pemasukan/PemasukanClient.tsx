...
interface Props { initialData: Pemasukan[]; role: Role }
...
function Modal({ onClose, onSave, editData }: {
  onClose: () => void
  onSave: (d: Omit<Pemasukan, 'id' | 'created_by' | 'created_at'>) => Promise<string | null>
  editData?: Pemasukan
}) {
      ...
      <div className="modal" style={{ padding: '28px' }}>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <div><label>Keterangan</label><input type="text" placeholder="Keterangan pemasukan..." value={form.keterangan} onChange={e => setForm(f => ({ ...f, keterangan: e.target.value }))} required /></div>
            ...
            <div>
              ...
              {Number(form.jumlah) > 0 && <div style={{ fontSize: '12px', color: '#10b981', marginTop: '4px', fontWeight: '600' }}>= {formatRupiah(Number(form.jumlah))}</div>}
            </div>
...
export default function PemasukanClient({ initialData, role }: Props) {
  ...
  async function handleSave(form: Omit<Pemasukan, 'id' | 'created_by' | 'created_at'>): Promise<string | null> {
    try {
      if (editItem) {
        const { data: updated } = await apiFetch('PATCH', { id: editItem.id, ...form })
        if (updated) setData(d => d.map(x => x.id === editItem.id ? updated : x))
        showToast('Pemasukan berhasil diupdate')
      } else {
  ...
  return (
    <div style={{ padding: '28px 32px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: '700', color: '#1e293b', marginBottom: '4px' }}>💰 Pemasukan</h1>
      ...
      <div style={{ background: 'linear-gradient(135deg, #10b981, #059669)', borderRadius: '16px', padding: '20px 24px', marginBottom: '20px', color: 'white', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
      ...
      <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
        <table>
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={isAdmin ? 5 : 4} style={{ textAlign: 'center', padding: '48px', color: '#94a3b8' }}>Belum ada data pemasukan</td></tr>
            ) : filtered.map(item => (
              <tr key={item.id}>
                ...
                <td style={{ textAlign: 'right', fontWeight: '600', color: '#10b981' }}>{formatRupiah(item.jumlah)}</td>
                {isAdmin && (
      ...
      {toast && (
        <div style={{ position: 'fixed', bottom: '24px', right: '24px', zIndex: 100, background: '#0f172a', color: 'white', borderRadius: '12px', padding: '14px 20px', fontSize: '14px', fontWeight: '500', boxShadow: '0 8px 24px rgba(0,0,0,0.3)', display: 'flex', alignItems: 'center', gap: '10px' }}>
          ✅ {toast}
        </div>
      )}
    </div>
