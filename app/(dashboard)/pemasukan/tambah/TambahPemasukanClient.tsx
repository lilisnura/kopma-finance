...
export default function TambahPemasukanClient() {
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: '760px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '28px' }}>
        <button onClick={() => router.push('/pemasukan')}
        ...
        <div>
          <h1 style={{ fontSize: '22px', fontWeight: '700', color: '#1e293b', marginBottom: '2px' }}>💰 Input Pemasukan</h1>
          <p style={{ color: '#64748b', fontSize: '13px' }}>Tambah data pemasukan koperasi</p>
        </div>
      </div>
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div><label>Keterangan <span style={{ color: '#ef4444' }}>*</span></label>
                <input type="text" placeholder="Contoh: Iuran anggota bulan Januari" value={form.keterangan} onChange={e => setForm(f => ({ ...f, keterangan: e.target.value }))} required />
              </div>
              ...
              <div>
                ...
                {Number(form.jumlah) > 0 && <div style={{ marginTop: '5px', fontSize: '13px', color: '#10b981', fontWeight: '600' }}>= {formatRupiah(Number(form.jumlah))}</div>}
              </div>
              ...
              <button type="button" onClick={() => router.push('/pemasukan')}
          ...
          {successList.length === 0
            ...
            : successList.map((item, i) => (
              <div key={i} style={{ padding: '11px 16px', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ fontSize: '13px', fontWeight: '700', color: '#10b981', whiteSpace: 'nowrap', marginLeft: '10px' }}>+{formatRupiah(item.jumlah)}</div>
              </div>
            ))}
        </div>
      </div>
